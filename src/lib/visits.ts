...
/** The shops a rep is nearest to, for a check-in screen that opens on one. */
export function nearestShops<
  T extends { latitude: number | null; longitude: number | null },
>(shops: T[], fix: Fix | null, limit = 5): { shop: T; metres: number | null }[] {
  const withDistance = shops.map((shop) => ({
    shop,
    metres:
      fix && shop.latitude !== null && shop.longitude !== null
        ? metresBetween(fix.latitude, fix.longitude, shop.latitude, shop.longitude)
        : null,
  }));

  // Pinned shops first, nearest first; unpinned ones after, since "unknown"
  // sorts below any real distance rather than above it.
  return withDistance
    .sort((a, b) => {
...
/**
 * Straight-line metres on a sphere — the same haversine the database uses.
 *
 * Here only to sort a list before anybody checks in. The distance that gets
 * *recorded* is the database's, computed from the shop's own row at the moment
 * of the check-in.
 */
